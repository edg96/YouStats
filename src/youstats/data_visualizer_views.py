...
class DataVisualizerViews(DataVisualizer):
    ...
    @staticmethod
    def _plot_views_single_channel(self, channel_data: dict[str, dict[str, int]], channel_name: str) \
            -> None:
        """
        ...
        Parameters:
            ...
            by year and month.
            channel_name (str): Name of the channel for labeling the plot.
        """
        ...
        for year in channel_years:
            ...
            plt.figure(num=f'{channel_name} - Views per month {year}')
            plt.plot(months, channel_values, marker='o', label=legend_channel)
            plt.title(f'Total Views per month in {year} - {channel_name}', fontsize=15)
            plt.xlabel('Months')
    ...
    # Calling functions
    def show_common_years_views(self) -> None:
        """
        ...
        common_years = self._extract_common_years(list(pivot_views_per_year.keys()),
                                                  list(targeted_views_per_year.keys()))

        self._plot_views_common_years(self, pivot_views_per_year, targeted_views_per_year, common_years)

    def show_single_channel_views(self, pivot_channel: bool, targeted_channel: bool) -> None:
        """
        Visualize the total views per month by the pivot and targeted YouTube channels.
        """
        if pivot_channel:
            ...
            self._plot_views_single_channel(self, pivot_views_per_year, "Pivot Channel")
        ...
        if targeted_channel:
            ...
            self._plot_views_single_channel(self, targeted_views_per_year, "Targeted Channel")
